...
This analysis explores the relationships of agricultural commodity loss, at a county level, from 2001-2015, across the three state region of the Pacific Northwest (Washington, Idaho, and Oregon), and then focuses on the Inland Pacific Northwest (INPW - 26 county region that encapsulates portions of Washington, Idaho, and Oregon). 

Here we explore the entire range of commodities and damage causes, identifying the top revenue loss commodities and their most pertinent damage causes - as indicated from the USDA's agricultural commodity loss insurance archive.  We also explore claim freqency, and finally zero in on the top 5 commodities for the region, and examine specific loss and claim frequency by year and damage cause.
...
<br></br>
<br></br>
**Step 8.** APPLES: IPNW region overview of Insurance Loss 2001-2015
<br></br>
<br></br>
**Step 9.** BARLEY: IPNW region overview of Insurance Loss 2001-2015
<br></br>
<br></br>
**Step 10.** WHEAT: IPNW region overview of Insurance Loss 2001-2015
<br></br>
<br></br>
**Step 11.** CHERRIES: IPNW region overview of Insurance Loss 2001-2015
<br></br>
<br></br>
**Step 12.** DRY PEAS: IPNW region overview of Insurance Loss 2001-2015
<br></br>
<br></br>
...
#write.csv(RMA_PNW, file = "/dmine/data/USDA/crop_indemnity_PNW/RMA_PNW_2001_2015.csv")
...
sumloss_palouse_sum <- RMA_damage_IPNW
count_palouse_count <- RMA_damage_IPNW
palouse_lossperacres <- RMA_damage_IPNW
...
##Step 3: PNW insurance claim loss summary
...
RMA_PNW_noneg3 <- subset(RMA_PNW_noneg2, month !="" )
  ...
  library(reshape2) ; RMA_damage_PNW_transformed_county <- dcast(RMA_PNW_noneg3, county + state ~ damagecause, value.var = c("lossperacre"), mean)

is.nan.data.frame <- function(x)
do.call(cbind, lapply(x, is.nan))

RMA_damage_PNW_transformed_county[is.nan(RMA_damage_PNW_transformed_county)] <- 0

RMA_damage_PNW_transformed_county$statecounty <- paste(RMA_damage_PNW_transformed_county$county, "_", RMA_damage_PNW_transformed_county$state, sep="")
rownames(RMA_damage_PNW_transformed_county) <- RMA_damage_PNW_transformed_county$statecounty
RMA_damage_PNW_exogenous_county <- RMA_damage_PNW_transformed_county[,3:31]



library(reshape2) ; RMA_damage_PNW_transformed_county <- dcast(RMA_PNW_noneg3, county + state ~ damagecause, value.var = c("loss"), sum)
...
RMA_damage_PNW_transformed_county$statecounty <- paste(RMA_damage_PNW_transformed_county$county, "_", RMA_damage_PNW_transformed_county$state, sep="")
rownames(RMA_damage_PNW_transformed_county) <- RMA_damage_PNW_transformed_county$statecounty
RMA_damage_PNW_exogenous_county <- RMA_damage_PNW_transformed_county[,3:32]
...
autoplot(prcomp(RMA_damage_PNW_exogenous, center = TRUE, scale = TRUE), data = RMA_damage_PNW_transformed, colour = 'county', loadings = TRUE, loadings.label = TRUE, loadings.label.size  = 3, label = FALSE, label.size = 3, legend = FALSE) + geom_text(vjust=-1, label=rownames(RMA_damage_PNW_exogenous)) + theme(legend.position = "none")
...
#PNW using county as group - with damage cause as loadings - lossperacre


fit <- princomp(RMA_damage_PNW_exogenous_month, cor=TRUE, scale = TRUE, center = TRUE)
autoplot(prcomp(RMA_damage_PNW_exogenous_county, scale = TRUE, center = TRUE), data = RMA_damage_PNW_transformed_county, colour = 'statecounty', loadings = TRUE, loadings.label = TRUE, loadings.label.size  = 3, label = FALSE, label.size = 3, legend = FALSE) + geom_text(vjust=-1, label=rownames(RMA_damage_PNW_exogenous_county)) + theme(legend.position = "none")
...
autoplot(prcomp(RMA_damage_IPNW_exogenous_2015, scale = TRUE, center = TRUE), data = RMA_damage_IPNW_transformed_2015, colour = 'county', loadings = TRUE, loadings.label = TRUE, loadings.label.size  = 3, label = FALSE, label.size = 3, legend = FALSE) + geom_text(vjust=-1, label=rownames(RMA_damage_IPNW_exogenous_2015)) + theme(legend.position = "none")

pca_RMA_damage_IPNW_exogenous$rotation

plot(pca_RMA_damage_IPNW_exogenous$rotation)

std_dev <- pca_RMA_damage_IPNW_exogenous_2015$sdev
