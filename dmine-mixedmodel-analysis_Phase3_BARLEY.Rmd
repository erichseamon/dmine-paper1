...
In Phase 3, we perform a BARLEY mixed modeling analysis using a two-step hurdle technique, for a selected set of damage causes.  The following analysis builds on Phases 1 and 2.
...
In our two part hurdle model, we identify zero values - that is, counties and years that have zero loss for particular damage causes for barley.  Previously we removed counties that we have determined have no barley being grown - based on known crop yield data.  The counties we are identifying are those where we KNOW barley are being grown, but in some instances, there are no loss claims being filed in particular years.  
...
###Barley Non-zero Goodness of fit hoslem test
...
chisq.test(alllevs2_barley$non_zero, fitted(m1))
...
###Barley zero/non-zero bionomal model to see outliers and zeros values vs non-zero values 
...
###barley binomal model summary for significant factors
...
###Barley mixed model coefficient estimates for damage cause and year
...
colnames(coef_barley_exp) <- c("factors", "lower", "upper", "odds_ratio", "stderror")

#library(lattice)
#var <- factor(rownames(coef_barley_exp[1:4]))
#boxplot(lower+upper~rn, data=coef_barley_exp, las = 2, horizontal = TRUE)

ggplot(coef_barley_exp, aes(x = factors, y = odds_ratio)) +
