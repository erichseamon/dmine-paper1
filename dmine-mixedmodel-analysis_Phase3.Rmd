...
ggplot(coef_apples_exp, aes(x = rn, y = estimate)) +
  ...
  geom_errorbar(aes(ymax = upper, ymin = lower)) + ggtitle("Palouse Region Apples \n Odds Ratios of Year and Damage Cause") 
```
<br></br>
...
ggplot(coef_cherries_exp, aes(x = rn, y = estimate)) +
  geom_point(size = 4) + coord_flip() +
...
coef_drypeas <- as.data.frame(summary(m2)$coefficients[,1:2])
coef_drypeas <- coef_drypeas[2:nrow(coef_drypeas),]
confint_drypeas <- as.data.frame(confint(m2))                        
confint_drypeas <- confint_drypeas[4:nrow(confint_drypeas),]

coef_drypeas2 <- cbind(confint_drypeas, coef_drypeas)
coef_drypeas_exp <- exp(coef_drypeas2)
colnames(coef_drypeas_exp) <- c("lower", "upper", "estimate", "stderror")
coef_drypeas_exp$lower <- as.numeric(coef_drypeas_exp$lower)
coef_drypeas_exp$upper <- as.numeric(coef_drypeas_exp$upper)
coef_drypeas_exp$estimate <- as.numeric(coef_drypeas_exp$estimate)
coef_drypeas_exp$stderror <- as.numeric(coef_drypeas_exp$stderror)

library(data.table)
setDT(coef_drypeas_exp, keep.rownames = TRUE)[]

#library(lattice)
#var <- factor(rownames(coef_drypeas_exp[1:4]))
#boxplot(lower+upper~rn, data=coef_drypeas_exp, las = 2, horizontal = TRUE)

ggplot(coef_drypeas_exp, aes(x = rn, y = estimate)) +
  geom_point(size = 4) + coord_flip() +
  geom_errorbar(aes(ymax = upper, ymin = lower)) + ggtitle("Palouse Region Dry Peas \n Odds Ratios of Year and Damage Cause")          
```