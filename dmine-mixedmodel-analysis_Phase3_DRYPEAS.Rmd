...
chisq.test(alllevs2_drypeas$non_zero, fitted(m1))
...
###Dry peas binomal model summary for only significant factors
...
###Dry Peas mixed model coefficient estimates for damage cause and year
...
###We examine the DRY PEAS coefficent estimates of our model output.
...
colnames(coef_drypeas_exp) <- c("factors", "lower", "upper", "odds_ratio", "stderror")

#library(lattice)
#var <- factor(rownames(coef_drypeas_exp[1:4]))
#boxplot(lower+upper~rn, data=coef_drypeas_exp, las = 2, horizontal = TRUE)

ggplot(coef_drypeas_exp, aes(x = factors, y = odds_ratio)) +
